package cn.wuxia.component.epay.chinapay.util.connection;
...
public abstract class CPHttpConnection {
    protected static final Logger logger = LoggerFactory.getLogger("epay");
    ...
    public int sendMsg(String msgStr) {
        ...
        try {
            ...
            throw new AppServiceException("连接失败,errCode=[-12];errMsg=[无法连接对方主机]");
        } catch (IOException ioe) {
            throw new AppServiceException("连接失败,errCode=[-14];errMsg=[通讯建链失败]");
        }
        ...
        try {
            ...
            throw new AppServiceException("Http连接失败,errCode=[-52];errMsg=[数据处理超时]");
        } catch (Exception e) {
            throw new AppServiceException("数据处理失败");
        } finally {
            try {
                if (tOut != null)
                    tOut.close();
            } catch (IOException e) {
                throw new AppServiceException("输出流关闭失败");
            }
        }
        ...
        try {
            ...
            logger.info("http ResponseCode=[" + respCode + "]");
            if (RESPCODE_SUCCESS == respCode) {
                ...
                if (contentLen == -1) {
                    logger.info("获取ContentLength失败，通过ByteArrayOutputStream方式获取数据");
                ...
                logger.warn(new StringBuffer("对方返回错误代码=[").append(respCode).append("];respMsg=[").append(urlCon.getResponseMessage()).append("]")
                        .toString());
            }

        } catch (IOException ioex) {
            throw new AppServiceException("连接失败,errCode=[-52];errMsg=[数据接收错误]");
        } catch (Exception ex) {
            throw new AppServiceException("系统处理异常");
        } finally {
            try {
                if (is != null)
                    is.close();
            } catch (IOException e) {
                throw new AppServiceException("输入流关闭失败");
            }
